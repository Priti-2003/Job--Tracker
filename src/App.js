...
const API =
  "https://job-tracker-backend-production-d896.up.railway.app/api/applications";
    ...
    <div style={s.root}>
      ...
      <div style={s.statsRow}>
        {[
          ...
          {
            label: "Active",
            val: jobs.filter((j) => !["Rejected", "Offer"].includes(j.status))
              .length,
            color: "#4E9FE5",
          },
      ...
      {showForm && (
        <div style={s.formBox}>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            <div style={s.statusGrid}>
              {STATUSES.map((st) => (
                ...
                >
                  <span style={{ ...s.dot, background: STATUS_COLORS[st] }} />
                  {st}
                </button>
              ))}
            </div>
          ...
          <button onClick={saveJob} style={s.saveBtn}>
            ...
            <div style={s.filterRow}>
              {["All", ...STATUSES].map((st) => (
                <button
                  ...
                  style={{
                    ...
                    ...(filterStatus === st
                      ? {
                          background:
                            st === "All" ? "#1e2d45" : STATUS_COLORS[st] + "22",
                          borderColor:
                            st === "All" ? "#4E9FE5" : STATUS_COLORS[st],
                ...
                >
                  ...
                  {st}
                  {st !== "All" && counts[st] > 0 && (
                    <span style={s.pill}>{counts[st]}</span>
                  )}
                </button>
              ))}
            </div>
              ...
              <tbody>
                {filtered.map((job) => (
                  <tr key={job.id} style={s.tr}>
                    ...
                    <td style={s.td}>
                      <button onClick={() => editJob(job)} style={s.editBtn}>
                        ✎ Edit
                      </button>
                      <button
            ...
            {STATUSES.map((st) => {
              ...
              return (
                <div key={st} style={s.barRow}>
                  <span style={{ ...s.barLabel, color: STATUS_COLORS[st] }}>
                    {st}
                  </span>
                  <div style={s.barBg}>
                    <div
          ...
          <div style={s.chartBox}>
            ...
            <div style={s.statsGrid}>
              {STATUSES.map((st) => (
                <div
                  key={st}
                  style={{ ...s.statTile, borderColor: STATUS_COLORS[st] }}
                >
                  <span
                    style={{
                      fontSize: 28,
          ...
          <div style={s.chartBox}>
            ...
            <div style={s.rateRow}>
              <div style={s.rateCard}>
                ...
                >
                  {jobs.length > 0
                    ? Math.round((counts["Offer"] / jobs.length) * 100)
                    : 0}
                  %
                </span>
              ...
              <div style={s.rateCard}>
                <span
                  style={{ fontSize: 32, fontWeight: 700, color: "#FBBF24" }}
                >
                  {jobs.length > 0
                    ? Math.round((counts["Interview"] / jobs.length) * 100)
                    : 0}
                  %
                </span>
                <span style={{ color: "#64748b", fontSize: 13 }}>
              ...
              <div style={s.rateCard}>
                <span
                  style={{ fontSize: 32, fontWeight: 700, color: "#F87171" }}
                >
                  {jobs.length > 0
                    ? Math.round((counts["Rejected"] / jobs.length) * 100)
                    : 0}
                  %
                </span>
                <span style={{ color: "#64748b", fontSize: 13 }}>
